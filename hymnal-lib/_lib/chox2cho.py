...
Parameters:
    -(ext|bas) -mode: 
                ext: - extended chords, 
                bas: basic chords (simplified)
    -h         - just preprocess chox to fill out all positional chords. Output: chox-formatted file.
                Good for debugging

Usage:
    python3 _lib/chox2cho.py chox/test/amazing_grace.chox -ext -h

Further processing:
    chordpro amazing_grace.cho --output amazing_grace.pdf [--generate=PDF]
    
"""
#python.exe _lib/chox2cho.py  chox/test/amazing_grace.chox -ext -h
# example of call
...
class ChoConverter():
    ...
    def check_f1(s: str)->(bool, list):
        ...
        if "|]" in s:
            string_valid = False
            report.append("Bad syntax: you must explicitly specify the dash "-" [chord|-] instead of just [chord|]")
        brackets_level = 0
        sep_in = 0
        in_brackets = False
        for c in s:
